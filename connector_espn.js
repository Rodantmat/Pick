import { withTimeout, parseMinutesValue, average, jinaUrl, norm, canon } from './utils_core.js';
...
async function fetchJson(url, headers = {}) {
  try {
    ...
    const proxy = await withTimeout(
      fetch(jinaUrl(url), { headers: { Accept: 'application/json,text/plain,*/*' } }),
...
function playerNameVariants(playerName = '') {
  const raw = norm(playerName);
  const parts = raw.split(/\s+/).filter(Boolean);
  const first = parts[0] || '';
  const last = parts[parts.length - 1] || '';
  const set = new Set([raw, canon(raw)]);
  if (first && last) {
    set.add(`${first} ${last}`);
    set.add(`${last}, ${first}`);
    set.add(`${first} ${last[0]}.`);
  }
  return [...set].map((s) => canon(s));
}

function matchAthlete(result, playerName) {
  const variants = playerNameVariants(playerName);
  const names = [
    result?.displayName,
    result?.name,
    result?.fullName,
    result?.position?.athlete?.displayName
  ].filter(Boolean).map(canon);
  return names.some((n) => variants.some((v) => n === v || n.includes(v)));
}

export async function searchEspnAthlete(playerName) {
  const q = encodeURIComponent(playerName);
  const url = `https://site.web.api.espn.com/apis/search/v2?query=${q}&limit=8&type=player`;
  const json = await fetchJson(url);
  const results = json.results || [];
  const picked = results.find((r) => matchAthlete(r, playerName)) || results[0];
...
export async function fetchEspnGameLog(playerName) {
  ...
  const category =
    seasonTypes?.[0]?.categories?.[0] ||
    seasonTypes?.find((x) => Array.isArray(x.categories) && x.categories.length)?.categories?.[0] ||
    null;
...
function statLookup(labels = [], stats = []) {
  const out = {};
  labels.forEach((lab, idx) => {
    out[String(lab || '').toUpperCase()] = stats[idx];
  });
  return out;
}

function mapEvent(event, labels) {
  const lookup = statLookup(labels, event.stats || []);
  ...
  return {
    ...
    threes: Number(lookup['3PM'] ?? lookup.FG3M ?? lookup['3PTM'] ?? NaN),
...
export function computePropAverage(events, propFamily, n) {
  ...
  const vals = sliced.map((ev) => {
    switch (propFamily) {
      ...
      case 'PRA':
      case 'Pts+Rebs+Asts': return (ev.points ?? 0) + (ev.rebounds ?? 0) + (ev.assists ?? 0);
