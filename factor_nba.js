...
import { ensureDiagState, upsertDiagEntry, rawPreview } from './diagnostics.js';
...
function baseCard(spec) {
  return {
    ...spec,
    status: spec.live ? 'loading' : 'waiting',
    statusLabel: spec.live ? 'Loading live probe...' : 'Paused',
    sourcesLabel: spec.sources.join(' • '),
    parsedResult: '—',
    evidence: spec.live ? '—' : spec.note
  };
}
...
function setCard(row, key, patch) {
  const card = row.miningStatus.find((c) => c.key === key);
  if (!card) return;
...
function asReady(val, provider = 'direct', evidence = '') {
  return {
    status: 'ready',
    statusLabel: provider === 'direct' ? 'Direct source found' : 'Search evidence found',
    parsedResult: String(val),
    evidence: evidence || String(val)
  };
}

function asFailed(msg = 'No trusted source return.') {
  return {
    status: 'failed',
    statusLabel: 'Probe failed',
    parsedResult: 'No trusted source return.',
    evidence: msg
  };
}

function queryFor(row, purpose) {
  return `${row.entity} ${purpose} nba`;
}
...
function parseAverageSentence(text, row) {
  ...
  const vals = {
    ...
    Rebounds: reb && Number(reb[1]),
    Assists: ast && Number(ast[1]),
    '3PT Made': threes && Number(threes[1])
  };

  if (p === 'Points') return vals.Points ?? null;
  if (p === 'Rebounds') return vals.Rebounds ?? null;
  if (p === 'Assists') return vals.Assists ?? null;
  if (p === '3PT Made') return vals['3PT Made'] ?? null;
  if ((p === 'PRA' || p === 'Pts+Rebs+Asts') && vals.Points != null && vals.Rebounds != null && vals.Assists != null) return vals.Points + vals.Rebounds + vals.Assists;
...
function bestSearchValue(text, row, key) {
  ...
  if (avgCombo) {
    const pts = Number(avgCombo[1]);
    const reb = Number(avgCombo[2]);
    const ast = Number(avgCombo[3]);
    if (row.propFamily === 'Points') return pts;
    if (row.propFamily === 'Rebounds') return reb;
    if (row.propFamily === 'Assists') return ast;
    if (row.propFamily === 'PRA' || row.propFamily === 'Pts+Rebs+Asts') return pts + reb + ast;
  ...
  if (avgThrees && row.propFamily === '3PT Made') return Number(avgThrees[1]);
  ...
  if (key === 'injury') {
    if (/out for season|\bout\b/i.test(cleaned)) return 0;
  ...
  if (key === 'schedule') {
    const yearCount = (cleaned.match(/\b20\d{2}\b/g) || []).length;
  ...
  for (const key of liveKeys) {
    try {
      if (key === 'validation') {
        setCard(row, key, {
          status: 'ready',
          statusLabel: 'Parsed from ingested row',
          parsedResult: `${row.entity} • ${row.lineText} • vs/@ ${row.opponent} • team ${row.team}`,
          evidence: 'Parsed from ingested row'
        });
        continue;
      }
      ...
      if (key === 'coverage') {
        ...
        setCard(row, key, {
          ...
          parsedResult: `live:${live} | paused:${paused}`,
          evidence: `live:${live} | paused:${paused}`
        });
        continue;
      }
      ...
      if (['last5', 'last10', 'last20'].includes(key) && espnBundle?.events?.length) {
        ...
        if (val != null) {
          setCard(row, key, { ...asReady((Math.round(val * 10) / 10).toFixed(1), 'direct'), sourcesLabel: 'ESPN direct' });
          continue;
        }
      }
      ...
      if (key === 'season') {
        ...
        if (val != null) {
          setCard(row, key, { ...asReady((Math.round(val * 10) / 10).toFixed(1), 'direct'), sourcesLabel: 'ESPN core/direct' });
          continue;
        }
      }
      ...
      if (key === 'career') {
        ...
        if (val != null) {
          setCard(row, key, { ...asReady((Math.round(val * 10) / 10).toFixed(1), 'search'), sourcesLabel: 'backup search • winner: ddg' });
          continue;
        }
      ...
      if (key === 'minutes' && espnBundle?.events?.length) {
        const mins = computeProjectedMinutes(espnBundle.events);
        if (mins != null) {
          setCard(row, key, { ...asReady(mins, 'direct'), sourcesLabel: 'Internal from ESPN history' });
          continue;
        }
      }
      ...
      if (key === 'injury' && injuryHtml) {
        ...
        if (status) {
          setCard(row, key, { ...asReady(status.score, 'direct', status.status), sourcesLabel: 'ESPN injuries' });
          continue;
        }
      }
      ...
      if (key === 'starters') {
        ...
        if (val != null) {
          setCard(row, key, { ...asReady(val, 'search', String(val)), sourcesLabel: 'backup search • winner: ddg' });
          continue;
        }
      }
      ...
      if (key === 'schedule' && espnBundle?.events?.length) {
        ...
        if (fatigue != null) {
          setCard(row, key, { ...asReady(fatigue, 'direct'), sourcesLabel: scoreboard ? 'NBA CDN scoreboard + ESPN history' : 'ESPN history', evidence: String(fatigue) });
          continue;
        }
      }
      ...
      if (key === 'pace') {
        ...
        if (metric?.pace != null) {
          setCard(row, key, { ...asReady(Math.round(metric.pace), 'direct', String(metric.pace)), sourcesLabel: nbaAdv ? 'NBA stats advanced' : 'Basketball-Reference backup' });
          continue;
        }
      }
      ...
      if (key === 'oppdef') {
        ...
        if (metric?.defRating != null) {
          setCard(row, key, { ...asReady(Math.round(metric.defRating), 'direct', String(metric.defRating)), sourcesLabel: nbaAdv ? 'NBA stats advanced' : 'Basketball-Reference backup' });
          continue;
        }
      }
      ...
      if (key === 'homeaway' && espnBundle?.events?.length) {
        ...
        if (currentAway !== null && currentAway !== undefined) {
          ...
          if (score != null) {
            setCard(row, key, { ...asReady(score, 'direct'), sourcesLabel: gc ? 'ESPN direct + scoreboard' : 'ESPN direct' });
            continue;
          }
        }
      }

      const fallbackPurposeMap = {
        last5: 'last 5 games',
        last10: 'last 10 games',
        last20: 'last 20 games',
        season: 'season averages',
        career: 'career stats per game',
        minutes: 'minutes',
        schedule: 'recent schedule nba',
        homeaway: 'home away splits nba',
        injury: 'injury status',
        starters: 'starting lineup expected lineup',
      ...
      setCard(row, key, {
        status: 'failed',
        statusLabel: 'Live probe failed',
        parsedResult: 'Mining failed.',
