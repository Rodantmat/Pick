...
export function normalizeNbaPropFamily(prop=''){
  const p = canon(prop);
  if (/\b3\s*pt|\b3pt|\b3pm|three/.test(p)) return '3PT Made';
  if (p==='pra' || p.includes('points rebounds assists') || p.includes('pts+rebs+asts')) return 'Pts+Rebs+Asts';
  if (p==='pra') return 'PRA';
...
function extractProp(text){
  const candidates = ['3PT Made','Rebs+Asts','Pts+Asts','Pts+Rebs','Pts+Rebs+Asts','PRA','Fantasy Score','Blocks','Steals','Blks+Stls','Turnovers','Rebounds','Assists','Points'];
  const lower = canon(text);
  for (const c of candidates){ if (lower.includes(canon(c))) return c; }
  if (/\b3\s*pt|\b3pt|\b3pm/i.test(text)) return '3PT Made';
  return '';
}
...
function parseChunk(chunk, idx, selectedLeagueIds){
  ...
  const leagueId = selectedLeagueIds[0] || 'nba';
