...
function splitIntoBlocks(text = '') {
  ...
  for (const line of lines) { current.push(line); if (/(?:\bLess\b\s*\bMore\b|\bMore\b\s*\bLess\b|\bMore\b|\bLess\b)$/i.test(line)) flush(); }
  flush();
...
function inferLeagueFromProp(prop, block, selectedIds) {
  ...
  if (['Significant Strikes', 'Takedowns', 'Fight Time'].includes(prop) && (selectedIds || []).includes('ufc')) return 'ufc';
  if (['Hits', 'Total Bases', 'Pitcher Strikeouts', 'Runs', 'RBIs', 'Outs Recorded', 'Home Runs', 'Hits Allowed'].includes(prop) && (selectedIds || []).includes('mlb')) return 'mlb';
  if (['Shots on Goal', 'Goalie Saves', 'Points', 'Assists', 'Goals', 'Blocked Shots', 'Hits'].includes(prop) && (selectedIds || []).includes('nhl')) return 'nhl';
  if (['Goalie Saves', 'Passes Attempted', 'Shots Assisted', 'Attempted Dribbles', 'Shots On Target', 'Shots', 'Tackles Won', 'Crosses', 'Clearances', 'Fouls Committed', 'Goals', 'Assists'].includes(prop) && (selectedIds || []).includes('soccer')) return 'soccer';
  if (['Birdies or Better', 'Strokes', 'Pars', 'Bogeys'].includes(prop) && (selectedIds || []).includes('golf')) return 'golf';
  if (['Finishing Position', 'Fastest Laps'].includes(prop) && (selectedIds || []).includes('nascar')) return 'nascar';
  if (['Points', 'Rebounds', 'Assists', 'Pts+Rebs', 'Pts+Asts', 'Rebs+Asts', 'PRA', 'Pts+Rebs+Asts', '3PT Made', 'Fantasy Score', 'Blocks', 'Steals', 'Blks+Stls', 'Turnovers'].includes(prop)) {
...
function parseBlock(block, selectedIds) {
  ...
  if (teamRoleName) { team = norm(teamRoleName[1]); entity = dedupeRepeatedEntity(teamRoleName[2]); }
  if (!entity) {
    const abbrThenName = raw.match(/\b([A-Z]{2,4})\s-\s[A-Z-]+\s+([A-Za-z0-9'().&\- ]+?)\s+(?:vs|@)\s/i);
    if (abbrThenName) { team = norm(abbrThenName[1]); entity = dedupeRepeatedEntity(abbrThenName[2]); }
  }
  if (!entity) {
...
function dedupeRows(rows = []) { const seen = new Map(); for (const row of rows) { if (row?.rowId && !seen.has(row.rowId)) seen.set(row.rowId, row); } return [...seen.values()]; }
...
export function ingestText(text, state) {
  ...
  const found = [];
  const lines = String(text || '').replace(/\r/g, '').split('\n');
  for (const line of lines) { if (line.includes('\t')) { const row = parseDesktopLine(line, selectedIds); if (row) found.push(row); } }
  const nonTabText = lines.filter((x) => !x.includes('\t')).join('\n');
  const blocks = splitIntoBlocks(nonTabText);
  for (const block of blocks) { const row = parseBlock(block, selectedIds); if (row) found.push(row); }
  const existing = new Map((state.cleanedRows || []).map((r) => [r.rowId, r]));
  let added = 0;
  for (const row of dedupeRows(found)) { if (!existing.has(row.rowId)) { existing.set(row.rowId, row); added += 1; } }
  ...
  if (!added) return { added: 0, message: 'No recognizable rows found for the leagues you selected. Try one clean chunk from one board section, or check that the right league is selected.' };
