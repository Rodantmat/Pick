...
export function normalizeNbaPropFamily(prop = '') {
  const p = canon(prop);
  if (!p) return '';
  if (/\b3\s*ptm\b|\b3ptm\b|\b3pm\b|\b3pt\b|\b3 pointers made\b|\bthree pointers made\b|\bthrees made\b/.test(p)) return '3PTM';
...
function buildRowId(leagueId, entity, team, prop, line, type, opponent = '') {
  return [leagueId, canon(entity), canon(team), canon(opponent), canon(prop), String(line || '').trim(), type].join('|');
}
...
function findPropCandidates(text, selectedIds) {
  ...
  matches.sort((a, b) => (b.key.length - a.key.length) || (a.idx - b.idx));
  return matches;
}
...
function inferLeagueFromProp(prop, block, selectedIds) {
  ...
  if (['Total Games', 'Break Points Won', 'Double Faults', 'Aces', 'Games Won', 'Sets Won'].includes(prop) && (selectedIds || []).includes('tennis')) return 'tennis';
  if (['Passing Yards', 'Rushing Yards', 'Receiving Yards', 'Receptions', 'Longest Reception', 'Touchdowns'].includes(prop)) {
    if ((selectedIds || []).includes('nfl')) return 'nfl';
    if ((selectedIds || []).includes('ncaaf')) return 'ncaaf';
  }
  if (['Significant Strikes', 'Takedowns'].includes(prop) && (selectedIds || []).includes('mma')) return 'mma';
  if (['Hits', 'Runs', 'RBIs', 'Bases', 'Strikeouts', 'Pitching Outs'].includes(prop) && (selectedIds || []).includes('mlb')) return 'mlb';
  if (['Goals', 'Assists', 'Points', 'Shots On Goal', 'Blocked Shots', 'Saves'].includes(prop) && (selectedIds || []).includes('nhl')) return 'nhl';
  if (['Shots', 'Shots On Target', 'Passes', 'Tackles'].includes(prop) && (selectedIds || []).includes('soccer')) return 'soccer';
  if (['Birdies', 'Pars', 'Bogeys', 'Strokes'].includes(prop) && (selectedIds || []).includes('golf')) return 'golf';
  if (['Points', 'Rebounds', 'Assists', 'Pts+Rebs', 'Pts+Asts', 'Rebs+Asts', 'PRA', 'Pts+Rebs+Asts', '3PTM', 'Fantasy Score', 'Blocks', 'Steals', 'Blks+Stls', 'Turnovers'].includes(prop)) {
    if ((selectedIds || []).includes('nba')) return 'nba';
    if ((selectedIds || []).includes('wnba')) return 'wnba';
    if ((selectedIds || []).includes('cbb')) return 'cbb';
  }
  if (low.includes('tennis') && (selectedIds || []).includes('tennis')) return 'tennis';
  if (low.includes('nhl') && (selectedIds || []).includes('nhl')) return 'nhl';
  if (low.includes('mlb') && (selectedIds || []).includes('mlb')) return 'mlb';
  if (low.includes('soccer') && (selectedIds || []).includes('soccer')) return 'soccer';
  if (low.includes('golf') && (selectedIds || []).includes('golf')) return 'golf';
  if (low.includes('mma') && (selectedIds || []).includes('mma')) return 'mma';
  if (low.includes('nfl') && (selectedIds || []).includes('nfl')) return 'nfl';
  if (low.includes('ncaaf') && (selectedIds || []).includes('ncaaf')) return 'ncaaf';
...
function parsePhoneTennisBlock(block, selectedIds) {
  ...
  if (lines.length < 6 || !/ - Player$/i.test(lines[0])) return null;
  ...
  if (!entity || !oppLine || !lineNum || !prop || !/\b(vs|@)\b/i.test(oppLine)) return null;
  const opp = oppLine.match(/\b(?:vs|@)\s+(.+?)(?:\s+(?:Mon|Tue|Wed|Thu|Fri|Sat|Sun)\b|\s+\d+m|\s+\d+h|$)/i);
  const opponent = opp ? norm(opp[1]) : '';
  return makeRow({ entity, team: '', opponent, prop, line: lineNum, leagueId: 'tennis', type: detectType(block), raw: normalized });
}
...
function parsePhoneStatBlock(block, selectedIds) {
  ...
  const lineNum = (lines[idx + 1] || '').match(/(\d+(?:\.\d+)?)/)?.[1] || '';
  const prop = norm(lines[idx + 2] || '');
  if (!oppLine || !lineNum || !prop || !/(?:\bvs\b|@)/i.test(oppLine)) return null;
  ...
  const opponent = opp ? norm(opp[1]) : '';
  return makeRow({ entity, team, opponent, prop, line: lineNum, leagueId, type: detectType(block), raw: norm(block) });
}

function parseDesktopLine(line, selectedIds) {
  const cols = String(line || '').split(/\t+/).map(norm).filter(Boolean);
...
function parseBlock(block, selectedIds) {
  ...
  if (!entity) {
    entity = dedupeRepeatedEntity(raw.split(/\s+/).slice(0, 5).join(' ').replace(/\b(Goblin|Demon|Taco|Free Pick)\b/ig, '').trim());
  }
  if (!team) {
...
export function ingestText(text, state) {
  ...
  state.cleanedRows = [...existing.values()].sort((a, b) => String(a.sport || '').localeCompare(String(b.sport || '')) || String(a.leagueId || '').localeCompare(String(b.leagueId || '')) || String(a.entity || '').localeCompare(String(b.entity || '')) || String(a.lineText || '').localeCompare(String(b.lineText || '')));
  if (!state.activeRowId && state.cleanedRows.length) state.activeRowId = state.cleanedRows[0].rowId;
  return added ? { added, message: `Ingested ${added} clean row${added === 1 ? '' : 's'}.` } : { added: 0, message: 'No recognizable rows found for the leagues you selected. Try one clean chunk from one board section, or check that the right league is selected.' };
}
