...
function miningStatusUi(status) {
  if (status === 'ready') return { cls: 'ready', icon: '✅' };
  if (status === 'failed') return { cls: 'failed', icon: '❌' };
  if (status === 'loading') return { cls: 'loading', icon: '🟡' };
...
function renderFeedStatus() {
  ...
  wrap.innerHTML = selectedLeagues().map((league) => {
    ...
    const chips = league.propCatalog.map((prop) => {
      const n = fed[league.id]?.[prop.toLowerCase().replace(/\s+/g, ' ').trim()] || fed[league.id]?.[prop] || 0;
...
function renderAnalysis() {
  ...
  if (!row) {
    ...
    hint.textContent = 'NBA live mining is wired from the stable v5.7.3 source chain.';
    rowCard.innerHTML = rowCardHtml(row);
    ensureNbaMining(row, requestRender);
    miningGrid.innerHTML = buildMiningStatus(row).map((card) => {
      const ui = miningStatusUi(card.status);
      return `
        <div class="mining-card">
          <h3>${escapeHtml(card.title)}</h3>
          <div class="mining-status ${ui.cls}">${ui.icon} ${escapeHtml(card.statusLabel)}</div>
    ...
    hint.textContent = 'League parsed correctly. NBA mining is the active live matrix in this recovery build.';
    rowCard.innerHTML = rowCardHtml(row);
  ...
  if (row && diagPanel) {
    ...
    diagPanel.innerHTML = keys.length
      ? keys.map((k) => `<div class="diag-entry"><div class="diag-key">${escapeHtml(k)}</div><div class="diag-mini">${escapeHtml(JSON.stringify(diag[k], null, 2))}</div></div>`).join('')
      : '<div class="message">No diagnostics yet.</div>';
  } else if (diagPanel) {
    diagPanel.innerHTML = '<div class="message">No diagnostics yet.</div>';
  }
}
...
function bindActions() {
  ...
  document.getElementById('dayScope')?.addEventListener('change', (event) => {
  ...
  document.getElementById('leagueChecklist')?.addEventListener('change', (event) => {
    const target = event.target;
  ...
  window.addEventListener('pickcalc:nba-update', (e) => {
    const row = activeRow();
    if (row && e.detail?.rowId === row.rowId) renderAnalysis();
  });
}
